...
class PlanckConstantSimulator(QMainWindow):
    def __init__(self):
        ...
        self.demo_button.clicked.connect(self.show_gif)  # 连接显示GIF的函数
    ...
    def calculate_results(self):
        ...
        for freq_THz in frequencies:
            ...
            stopping_voltage_with_noise = stopping_voltage + noise

            # 在表格中显示结果
            row_position = self.data_table.rowCount()
            self.data_table.insertRow(row_position)
            self.data_table.setItem(row_position, 0, QTableWidgetItem(f"{freq_THz:.2f}"))
            self.data_table.setItem(row_position, 1, QTableWidgetItem(f"{stopping_voltage_with_noise:.2f}"))

            # 保存数据
            self.frequency_data.append(freq_THz)
            self.stopping_voltage_data.append(stopping_voltage_with_noise)
    ...
    # 显示GIF动图
    def show_gif(self):
        gif_path = os.path.join(os.path.dirname(__file__), '光电效应动.gif')
        
        # 检查文件是否存在
        if not os.path.exists(gif_path):
            print("GIF文件不存在: ", gif_path)
            return

        # 调试信息，确保路径正确
        print("加载GIF文件: ", gif_path)

        # 创建一个窗口显示GIF
        self.gif_window = QWidget()
        self.gif_window.setWindowTitle("光电效应原理演示")

        # 创建一个QLabel来显示GIF
        self.gif_label = QLabel(self.gif_window)
        ...
        # 调整窗口大小适应GIF
        self.gif_window.setGeometry(300, 300, 600, 400)  # 根据GIF的大小调整
